...
t.eq(tokens[0].loc.start.line, 0, "The first token should be at line 0");
t.eq(tokens[0].loc.start.column, 0, "The first token should be at column 0");
t.eq(tokens[0].loc.end.column, 4, "Token columns should increase");
t.eq(tokens[1].type, 2, "Attribute reference token");
t.eq(tokens[1].value.characters, ">",
    "Attribute reference token contains characters");
t.eq(tokens[1].value.value, "&gt;",
...
t.eq(tokens[3].loc.start.column, 12, "EOF token starts at the end");
t.eq(tokens[3].loc.end.column, 12, "EOF token ends at the end");
